...
export function CouponCampaignsPage() {
  ...
  const getDiscountText = (coupon: any) => {
    if (coupon.type === "percentage") {
      return t.customers.customer.coupon.list.discount.percentage.replace(
        "{value}",
        coupon.value,
      );
    } else if (coupon.type === "fixed") {
      return t.customers.customer.coupon.list.discount.fixed.replace(
        "{value}",
        coupon.value,
      );
      ...
      return t.customers.customer.coupon.list.discount.points.replace(
        "{value}",
        coupon.value,
      );
    }
  ...
  const getUsageText = (coupon: any) => {
    const countText = t.customers.customer.coupon.list.usage.count.replace(
      "{count}",
      coupon.usageCount,
    );
      ...
      >
        <div>
          <h1 className="text-2xl font-semibold">
            {t.customers.customer.coupon.title}
          </h1>
      ...
      >
        <Table className={paginatedCoupons.length > 0 ? "rounded-b-none" : ""}>
          <TableHeader>
            <TableRow>
              <TableHead>
                {t.customers.customer.coupon.list.columns.campaign}
              </TableHead>
              <TableHead>
                {t.customers.customer.coupon.list.columns.discount}
              </TableHead>
              <TableHead>
                {t.customers.customer.coupon.list.columns.usage}
              </TableHead>
              <TableHead>
                {t.customers.customer.coupon.list.columns.duration}
              </TableHead>
              <TableHead>
                {t.customers.customer.coupon.list.columns.status}
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {paginatedCoupons.length === 0 ? (
              ...
              paginatedCoupons.map((coupon) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant="secondary">{getDiscountText(coupon)}</Badge>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">{getUsageText(coupon)}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">
                      <p>{coupon.startDate.toLocaleDateString()}</p>
                      <p className="text-muted-foreground">
                        {t.customers.customer.coupon.list.duration.to}{" "}
        ...
        <motion.div
          className={cn("border-t p-4 bg-main rounded-b-lg", {
            hidden: paginatedCoupons.length === 0,
          })}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
