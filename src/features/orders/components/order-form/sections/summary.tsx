...
export function Summary({ form }: SummaryProps) {
  ...
  const [discountBase, setDiscountBase] = useState<'subtotal' | 'total'>('subtotal');
  ...
  const handleDiscountBaseChange = (base: 'subtotal' | 'total') => {
    ...
    const newBaseAmount = base === 'subtotal' ? subtotal : (subtotal + shipping + currentTax);
  ...
  useEffect(() => {
    ...
    // Calculate total
    const calculatedTotal = subtotal + shipping + calculatedTax - calculatedDiscount;
  ...
  }, [form, subtotal, shipping, calculatedDiscount, isTaxEnabled, taxType, taxPercentage, taxValue]);
