...
const SHIPPING_METHODS = [
  { id: "standard", name: "Standard Shipping", price: 5.0 },
  { id: "express", name: "Express Shipping", price: 15.0 },
  { id: "overnight", name: "Overnight Shipping", price: 25.0 },
];
...
export function DynamicShipping({
  form,
  currentShipping,
}: DynamicShippingProps) {
  // Find the matching shipping method or default to standard
  const currentMethod =
    SHIPPING_METHODS.find((m) => m.price === currentShipping) ||
    SHIPPING_METHODS[0];
            ...
            <SelectContent>
              {SHIPPING_METHODS.map((method) => (
                <SelectItem key={method.id} value={method.id}>
                  {method.name} - $
                  {hasFreeShippingCoupon ? 0 : method.price.toFixed(2)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
