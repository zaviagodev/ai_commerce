...
export function Shipping({ form }: ShippingProps) {
  ...
  const [shippingType, setShippingType] = useState<"dynamic" | "manual">(
    "manual",
  );
  ...
  const handleShippingTypeChange = (type: "dynamic" | "manual") => {
    ...
    if (type === "dynamic" && prevType === "manual") {
      form.setValue("shipping", 5.0); // Standard shipping cost
    }
  };
  ...
  return (
    <Card>
      ...
      <CardContent>
        {shippingType === "dynamic" ? (
          <DynamicShipping form={form} currentShipping={currentShipping} />
        ) : (
          <ManualShipping form={form} currentShipping={currentShipping} />
        )}
      </CardContent>
    </Card>
  );
