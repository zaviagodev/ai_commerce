...
const ConditionOperatorSchema = z.enum(['greater_than', 'less_than', 'equal_to']);

// Condition type validation
const ConditionTypeSchema = z.enum(['total_spent', 'order_count', 'last_order', 'location']);
...
const OrderCountConditionSchema = BaseConditionSchema.extend({
  ...
  value: z.string().refine((val) => !isNaN(parseInt(val)) && parseInt(val) >= 0, {
