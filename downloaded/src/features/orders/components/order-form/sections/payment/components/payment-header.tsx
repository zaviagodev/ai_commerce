...
export function PaymentHeader({ isCancelled, isPaid, isShipped, isProcessing }: PaymentHeaderProps) {
  return (
    <div className="relative px-6 py-4 border-b border-gray-700/50 z-10">
      ...
      <div className="relative z-10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Badge
            variant={isCancelled ? "destructive" : (isPaid || isShipped ? "success" : "warning")}
            className="flex items-center gap-1.5"
          >
            <span className="relative flex h-2 w-2">
              <span className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 ${
                isCancelled ? 'bg-red-400' : (isPaid || isShipped ? 'bg-green-400' : 'bg-yellow-400')
              }`} />
              <span className={`relative inline-flex h-2 w-2 rounded-full ${
                isCancelled ? 'bg-red-500' : (isPaid || isShipped ? 'bg-green-500' : 'bg-yellow-500')
              }`} />
            </span>
            {isCancelled ? 'Cancelled' : (isPaid || isShipped ? 'Paid' : 'Unpaid')}
          </Badge>
